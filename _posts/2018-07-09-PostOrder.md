...
<!--more-->
## 1. 문제
호엽이가 수식에 대한 후위 순회동작 방식을 공부하고 해당 아고리즘을 응용한 수식 트리를 작성하여 확인하고자 한다.

### (1) 문제 제시
이진트리 후위 순회 알고리즘을 이용하여 수식 트리를 작성허ㅏ라. [예를 들어 (3 + 4) * 8은 중위 순회 방식이고, 앞 수식을 후위 순회 방식으로 바꾸면 3 4 + 8 *이 된다]
